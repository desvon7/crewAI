...
const NavLogo = () => <img className='h-8 w-8' src={logo} alt='CrewAI Visual Builder' />;
      ...
      <nav className='flex items-center justify-between p-6 lg:px-8' aria-label='Global'>
        <div className='flex items-center lg:flex-1'>
          <WaspRouterLink
            ...
            className='flex items-center -m-1.5 p-1.5 text-gray-900 duration-300 ease-in-out hover:text-orange-600'
          >
            <NavLogo />
            {isLandingPage && (
              <span className='ml-2 text-sm font-semibold leading-6 dark:text-white'>CrewAI Visual Builder</span>
            )}
          </WaspRouterLink>
        </div>
        ...
        <div className='hidden lg:flex lg:flex-1 gap-3 justify-end items-center'>
          ...
          {isUserLoading ? null : !user ? (
            <WaspRouterLink to={routes.LoginRoute.to} className='text-sm font-semibold leading-6 ml-3'>
              <div className='flex items-center duration-300 ease-in-out text-gray-900 hover:text-orange-600 dark:text-white'>
        ...
        <Dialog.Panel className='fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white dark:text-white dark:bg-boxdark px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10'>
          <div className='flex items-center justify-between'>
            <WaspRouterLink to={routes.LandingPageRoute.to} className='-m-1.5 p-1.5'>
              <span className='sr-only'>CrewAI Visual Builder</span>
              <NavLogo />
            </WaspRouterLink>
            <button
              type='button'
              ...
              <div className='py-6'>
                {isUserLoading ? null : !user ? (
                  <WaspRouterLink to={routes.LoginRoute.to}>
                    <div className='flex justify-end items-center duration-300 ease-in-out text-gray-900 hover:text-orange-600 dark:text-white'>
...
function renderNavigationItems(
  ...
  const menuStyles = cn({
    '-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50 dark:text-white dark:hover:bg-boxdark-2':
      !!setMobileMenuOpen,
    'text-sm font-semibold leading-6 text-gray-900 duration-300 ease-in-out hover:text-orange-600 dark:text-white':
      !setMobileMenuOpen,
  });
...
function Announcement() {
  return (
    <div className='flex justify-center items-center gap-3 p-3 w-full bg-gradient-to-r from-[#FF6600] to-[#fc0] font-semibold text-white text-center z-49'>
      <p
        onClick={() => window.open('https://github.com/crew-ai/crewai', '_blank')}
        className='hidden lg:block cursor-pointer hover:opacity-90 hover:drop-shadow'
      >
        CrewAI Visual Builder is open-source! ⭐️
      </p>
